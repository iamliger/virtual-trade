...
class TradingApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("AI 실전 단타 시뮬레이터 v2.6 (컴팩트 에디션)")

        # 💡 [변경] 해상도를 1100x750으로 축소하여 모든 모니터에서 보이도록 조정
        self.geometry("1100x750")
        create_tables()

        # [1] 상단: AI 시장 분석 (높이 축소)
        self.top_frame = ctk.CTkFrame(self, fg_color="#1a1a1a", height=80)
        self.top_frame.pack(fill="x", padx=15, pady=5)
        self.predict_box = ctk.CTkTextbox(
            self.top_frame, height=70, font=("Malgun Gothic", 12), text_color="#FFD700"
        )
        self.predict_box.pack(fill="x", padx=10, pady=5)
        ...
        self.ai_report_text = ctk.CTkTextbox(self.ai_panel, font=("Malgun Gothic", 14))
        self.ai_report_text.pack(fill="both", expand=True, padx=5, pady=5)

        # 오른쪽: 통계 전광판 및 설정 (가로 폭 유지)
        ...
        self.today_profit_label.pack(pady=2)
        ...
        # [3] 하단 탭 (높이 축소)
        self.tab_view = ctk.CTkTabview(self, height=180)
        ...
        # [4] 시작 버튼 (가장 중요 - 해상도 축소로 이제 확실히 보입니다)
    ...
    def update_ui(self, data):
        ...
        if data["status"] == "ACTIVE":
            ...
            self.today_profit_label.configure(
                text=f"오늘 수익: {data['today_profit']:,}원"
            )
            ...
            report = f"🎯 대상: {self.ticker_menu.get()} | 결정: [{data['decision']}]\n"
            report += f"상태: {data['trade_status']}\n"
            report += "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            report += f"💡 근거: {data['reason']}\n\n📰 뉴스:\n{data['news']}"
    ...
    def refresh_db_tabs(self):
        ...
        for h in holdings:
            self.holdings_box.insert("end", f"📦 {h[0]} | {h[1]}주 | {h[2]:,}원\n")
