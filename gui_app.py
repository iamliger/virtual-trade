...
class TradingApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("AI 실전 트레이딩 스테이션 v7.5 (DB-Driven)")
        ...
        self.predict_box.insert("0.0", "🚀 AI 지능형 종목 발굴 시스템 가동 대기 중\n")
        ...
        ctk.CTkLabel(
            self.stat_panel, text="💰 시드머니 (원)", font=("Malgun Gothic", 11)
        ).pack()
        ...
        ctk.CTkLabel(
            self.stat_panel, text="🎯 목표 수익 (원)", font=("Malgun Gothic", 11)
        ).pack()
                ...
                self.after(
                    ...
                    lambda: (
                        ...
                        self.add_predict_log(
                            f"💰 {amt:,}원 맞춤형 저가주 {len(new_list)}종목 발굴 완료"
                        ),
                    ),
                )
            except:
    ...
    def confirm_reset(self):
        reset_db_completely()
        self.refresh_ui_from_db()
        self.add_predict_log("☢️ 시스템 전체 리셋 및 10만 원 초기화 완료")
    ...
    def refresh_ui_from_db(self):
        ...
        h_header = f"{'거래시간':<18} | {'종목명':<12} | {'구분':<4} | {'가격':>10} | {'수량':>4} | {'실현손익':>10}\n"
